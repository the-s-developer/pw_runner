...
def patch_sync_browser_launch():
    try:
        print("playwright patch-------------------->")
        from patchright.sync_api import _api
        sync_browser_type = _api.SyncBrowserType
    except (ImportError, AttributeError):
        from patchright._impl._browser_type import BrowserType as sync_browser_type
    ...
    def launch_persistent_with_env(self, *args, **kwargs):
        print("PATCHED launch_persistent_with_env called! args:", args, "kwargs:", kwargs)
        chrome_path = os.environ.get("CHROME_PATH")
        patch_user_data_dir = os.environ.get("USER_DATA_DIR")

        print("USER_DATA_DIR",patch_user_data_dir)
        print("--->",kwargs)
